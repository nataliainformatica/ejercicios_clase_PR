...
public class App {
    ...
    public static void main(String[] args) {
        ...
        DiaSemana dia;
        int resultado;
        // PRIMERA PRUEBA DEL MÉTODO calcularNumero...
        try {
            dia = introducirDia();
            // SIGUIENTE CÓDIGO
            // CALCULAR DIAS
            ...
            System.out.println("LA fecha actual tiene " + calcularNumeroConExcepcion(dia));

        } catch (IllegalArgumentException ex) {
            System.out.println("NO VALE EL DIA");
        } catch (MesLargoException ex){
            System.out.println("El mes : " + ex.getMes() 
            + " tiene más de 4 " + ex.getDia());
        }
     ...
     */
     ...
     public static int calcularNumeroConExcepcion(DiaSemana diaSemana){
        ...
        Month mesM = fechaActual.getMonth();
        int anio = fechaActual.getYear();
        int mes = mesM.ordinal()+1; 
     
        int numero ; 
        
        
        numero = calcularNumero(diaSemana, mes, anio);
        if(numero >4 ){
            throw new MesLargoException(mes, diaSemana); 
        }
